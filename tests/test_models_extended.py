...
from fmsim.models import (
    VehicleParams, BicycleKinematic, pure_pursuit_control, stanley_control,
...
class TestBicycleKinematicExtended:
    ...
    def test_noise_injection(self):
        ...
        assert np.std(yaws) > 0.01  # Should have some variation
    ...
    def test_latency_buffer(self):
        ...
        u = (0.5, 0.0)  # Steering command
        
        # First few steps should not show steering effect
        for _ in range(3):
            new_state = model.step(state, (0.0, 0.0), 0.1)
            assert abs(new_state[2] - state[2]) < 1e-6  # No yaw change yet
            state = new_state
...
class TestStanleyControl:
    ...
    def test_stanley_straight_path(self):
        ...
        assert abs(steer) > 0.01  # Should produce steering command
...
class TestMPPIController:
    ...
    def test_mppi_empty_path(self):
        ...
        steer, idx = controller.control(state, path)
...
class TestTelemetryLogger:
    ...
    def test_logger_initialization(self):
        ...
        expected_keys = ['time', 'x', 'y', 'yaw', 'velocity', 'steering', 
    ...
    def test_logging_with_path(self):
        ...
        assert logger.data['cross_track_error'][0] > 0  # Off path
    ...
    def test_statistics_calculation(self):
        ...
        # Log some data with known statistics
        ...
        assert abs(stats['velocity']['mean'] - 12.5) < 1e-6
        assert stats['velocity']['min'] == 5.0

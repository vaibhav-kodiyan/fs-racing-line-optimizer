...
def run_animation(sim):
    ...
    speed_hist, curv_hist = [], []
    ax_hist, ay_hist, gz_hist, t_hist = [], [], [], []
    paused = {"v": False}
    ...
    def _set_xy(line, arr):
        try:
            if arr is None:
                line.set_data([], [])
            elif (hasattr(arr, "ndim") and arr.ndim == 2 and
                  arr.shape[1] >= 2 and arr.shape[0] >= 1):
    ...
    def init():
        ...
        return (coneL_sc, coneR_sc, center_ln, car_poly, speed_ln,
                curv_ln, imu_ln1, imu_ln2, imu_ln3)
    ...
    def update(_):
        ...
        try:
            ...
            print("Exception in simulation update:", repr(e))
            traceback.print_exc()
            fig.suptitle("Error: {}".format(e))
            paused["v"] = True
            return ()
        L = frame["cones_left"]
        R = frame["cones_right"]
        ...
        _set_xy(coneR_sc, R)
        _set_xy(center_ln, C)
        _set_xy(car_poly, car)
        t_hist.append(met["t"])
        speed_hist.append(met["speed"])
        curv_hist.append(met["curvature_mean"])
        ax_hist.append(met["ax"])
        ay_hist.append(met["ay"])
        gz_hist.append(met["gz"])
        ...
        return (coneL_sc, coneR_sc, center_ln, car_poly, speed_ln,
                curv_ln, imu_ln1, imu_ln2, imu_ln3)
    # Set a large but finite number of frames to cache
    # (1000 frames = ~30 seconds at 30fps)

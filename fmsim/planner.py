...
def _apply_corridor_constraints(path, corridor):
    ...
    for i, point in enumerate(path):
        ...
        if len(left_cones) > 0 and len(right_cones) > 0:
            ...
            corridor_center = 0.5 * (nearest_left + nearest_right)
            corridor_width = np.linalg.norm(nearest_right - nearest_left)
...
def optimization_based_racing_line(left_cones, right_cones, num_points=50, 
    ...
    try:
        ...
        objective = cp.Minimize(curvature_weight * curvature_cost + 

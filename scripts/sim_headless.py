...
from fmsim.utils import load_cones_json, car_triangle  # noqa: E402
from fmsim.planner import (laplacian_smooth, curvature_discrete,  # noqa: E402
                           pair_cones_to_midline)
from fmsim.models import (VehicleParams, BicycleKinematic,  # noqa: E402
...
def run_once(cones_path):
    ...
    while i_track < len(path) - 2:
        ...
        last_frame = {"L": left, "R": right, "C": path,
                      ...
                      "curv_mean": float(np.mean(
                          curvature_discrete(path))),
                      "speed": float(state[3])}
    return last_frame
...
def main():
    ...
    ax.set_aspect("equal")
    ax.legend(loc="best")
    ax.set_title("Track snapshot")
    out_png = os.path.join(args.out, "track.png")
    fig.tight_layout()
